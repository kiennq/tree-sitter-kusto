LANGUAGE_NAME := tree-sitter-kusto
HOMEPAGE_URL := https://github.com/kiennq/tree-sitter-kusto
VERSION := 1.0.0
DESCRIPTION := Kusto Query Language (KQL) grammar for tree-sitter
...
install: all
	install -d '$(DESTDIR)$(DATADIR)'/tree-sitter/queries/kusto '$(DESTDIR)$(INCLUDEDIR)'/tree_sitter '$(DESTDIR)$(PCLIBDIR)' '$(DESTDIR)$(LIBDIR)'
...
ifneq ($(wildcard queries/*.scm),)
	install -m644 queries/*.scm '$(DESTDIR)$(DATADIR)'/tree-sitter/queries/kusto
endif
...
ifneq ($(findstring mingw32,$(MACHINE)),)
	...
	$(RM) -r '$(DESTDIR)$(DATADIR)'/tree-sitter/queries/kusto
